...
class AttentiveCNN(nn.Module):
    ...
    def __init__(self, model_path, num_classes):
        ...
        if model_path is "":
            ...
            # modules = list(models.vgg16_bn(pretrained=True).features)[:40]
...
class Decoder(nn.Module):
    def __init__(self, vis_dim, vis_num, embed_dim, hidden_dim, vocab_size, num_classes, num_layers=1,
                 dropout_ratio=0.5):
        ...
        # attention
        self.att_vw = nn.Linear(self.vis_dim+self.vis_dim, self.vis_dim, bias=False)
        ...
        self.att_vw_2 = nn.Linear(num_classes, self.vis_dim, bias=False)
    ...
    def _attention_layer(self, features, hiddens, features_2, pattern):
        ...
        :return:
        """
        features_2 = self.att_vw_2(features_2).unsqueeze(1)
        features_2 = features_2.expand(features_2.size(0), features.size(1), features_2.size(2), )
        feas = torch.cat([features, features_2], 2)
        feas = self.att_vw(feas)
        att_full = hiddens.unsqueeze(1) * feas
...
class Encoder2Decoder(nn.Module):
    def __init__(self, args, vocab_size):
        ...
        self.decoder = Decoder(vis_dim, vis_num, embed_dim, hidden_dim, vocab_size, num_classes, num_layers,
                               dropout_ratio)
    ...
    def forward(self, images, captions, lengths, pattern):
        ...
        scores = self.decoder(features, probs, captions, lengths, pattern)
